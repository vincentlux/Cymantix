...
}' http://104.248.61.45:8983/solr/test/schema


curl -X POST -H 'Content-type:application/xml' --data-binary '{"add-field":{"name":"from","type":"text_general","stored":true }}' http://104.248.61.45:8983/solr/test/schema
curl -X POST -H 'Content-type:application/xml' --data-binary '{"add-field":{"name":"to","type":"text_general","stored":true }}' http://104.248.61.45:8983/solr/test/schema
...
curl -X POST -H 'Content-type:application/xml' --data-binary '{"add-field":{"name":"date","type":"tdate","stored":true }}' http://104.248.61.45:8983/solr/test/schema
